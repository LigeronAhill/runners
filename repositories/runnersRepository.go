...
func NewRunnersRepository(dbHandler *sql.DB) *RunnersRepository {
	return &RunnersRepository{dbHandler: dbHandler}
}
...
func (rr RunnersRepository) GetRunner(runnerId string) (*models.Runner, *models.ResponseError) {
	...
	defer rows.Close()
	var (
		id, firstName, lastName, country string
		personalBest, seasonBest         sql.NullString
		age                              int
		isActive                         bool
	)
	for rows.Next() {
...
func (rr RunnersRepository) GetRunnersByYear(year int) ([]*models.Runner, *models.ResponseError) {
	query := `
		SELECT runners.id, runners.first_name, runners.last_name, runners.age, runners.is_active, runners.country, runners.personal_best, runners.race_result
		FROM runners
		INNER JOIN (
			SELECT runner_id,
